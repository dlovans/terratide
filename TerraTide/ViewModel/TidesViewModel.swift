...
class TidesViewModel: ObservableObject {
    ...
    func attachTidesListener(for userLocation: Coordinate, userId: String) {
        let newTidesListener = tidesRepository.attachTidesListener(for: userLocation, userId: userId) { tides in
            if let tides {
                self.tides = tides
            } else {
